<?php
    ...
    $foto = isset($_FILES["file"]) ? $_FILES["file"] : NULL;
        ...
        if($empl_Modificar == "ok"){
            ...
            $auxFoto =  isset($foto["name"]) ? $foto["name"] : null;
            
            /* if(pathinfo("../fotos/" . $foto["name"], PATHINFO_EXTENSION) == ""){ */
            if($auxFoto == null){
        ...
        if($proceso == "Alta"){        
            ...
            if (file_exists($pathDestino)) {        
                ...
                if ($uploadOk === FALSE) {
                    ...
                    if (move_uploaded_file($foto["tmp_name"], $pathDestino)) {
